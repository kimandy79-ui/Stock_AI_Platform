...
def _load_tickers_from_file(path: "Path") -> list[Any]:
    ...
    :func:`app.services.universe.ticker_file_loader.load_tickers_from_file`
    (extracted so the dashboard's CSV-sourced pipeline trigger can reuse the
    same parsing logic without duplicating it). Kept as a module-level
    function here, under its original name, so existing call sites and tests
