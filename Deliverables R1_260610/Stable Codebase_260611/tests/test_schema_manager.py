...
EXPECTED_SIM_TABLES: frozenset[str] = frozenset(
    {
        "schema_versions",
        # Config tables shared with prod/debug (M21 Config Management Addendum §4).
        "strategy_configs",
        "runtime_configs",
        "config_activation_log",
        "sector_alias_map",
...
EXPECTED_COLUMNS: dict[str, list[str]] = {
    ...
    "pipeline_runs": ["run_id", "run_date", "run_type", "status", "started_at", "completed_at", "duration_sec", "steps_completed", "error_message", "strategy_config_ids_json", "runtime_config_ids_json", "config_snapshot_hash", "created_at"],
    ...
    "strategy_configs": ["config_id", "db_role", "strategy_name", "version", "parent_config_id", "config_json", "config_hash", "active_flag", "created_at", "created_by", "notes"],
    "runtime_configs": ["config_id", "db_role", "config_type", "version", "parent_config_id", "config_json", "config_hash", "active_flag", "created_at", "created_by", "notes"],
    "config_activation_log": ["activation_id", "config_id", "db_role", "config_type", "profile_name", "activated_at", "activated_by", "reason"],
    "sector_alias_map": ["source", "raw_sector", "canonical_sector", "active_flag", "created_at"],
...
# Tables shared between prod/debug and simulation (config tables, M21 addendum).
_SHARED_CONFIG_TABLES: frozenset[str] = frozenset(
    {"strategy_configs", "runtime_configs", "config_activation_log", "sector_alias_map"}
)

# For column-test role routing: prod-only = prod tables minus schema_versions and
# shared config tables; sim-only = sim-specific tables (not shared with prod).
_PROD_ONLY_TABLES: frozenset[str] = (
    EXPECTED_PROD_TABLES - {"schema_versions"} - _SHARED_CONFIG_TABLES
)
_SIM_ONLY_TABLES: frozenset[str] = (
    EXPECTED_SIM_TABLES - {"schema_versions"} - _SHARED_CONFIG_TABLES
)
