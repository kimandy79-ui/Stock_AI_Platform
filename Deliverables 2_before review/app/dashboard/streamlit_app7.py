...
def _render_daily_proposals() -> None:  # noqa: C901
    ...
    if latest:
        ...
        # Shorten verbose status strings for the chip
        status_short = {
            "success":               "success ✓",
    ...
    # Layout: title | chip (flex-grows) | button (fixed)
    st.markdown(
        f'<div style="display:flex;align-items:center;justify-content:space-between;'
        f'gap:12px;margin-bottom:12px;flex-wrap:nowrap">'
        f'<div class="da-title" style="flex-shrink:0">Daily Proposals</div>'
        f'<div style="display:flex;align-items:center;gap:10px;flex:1;justify-content:flex-end;min-width:0">'
        f'<div class="da-chip" style="flex:1;min-width:0;max-width:600px;'
        f'overflow:hidden;text-overflow:ellipsis;white-space:nowrap;height:38px;'
        f'display:flex;align-items:center;padding:0 14px">{chip_label}</div>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
    run_clicked = st.button("▶  Run Pipeline", key="dp_run", type="primary")
