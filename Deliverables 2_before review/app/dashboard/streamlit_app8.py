...
def _enrich_proposals(rows: list[dict], signal_date: date, db_role: str) -> list[dict]:
    ...
    # Fill missing company names from yfinance (best-effort, silent on failure)
        ...
        try:
            ...
            for t in missing_names:
                try:
                    ...
                    name = getattr(info, "long_name", None) or getattr(info, "short_name", None) or ""
                    company_map[t] = name
                except Exception:
                    pass
        except Exception:
...
def _date_picker_with_highlights(
    ...
    """Custom HTML/JS calendar that marks proposal dates with blue circles.

    Falls back to the session-state value if component returns nothing.
    """
    ...
    highlighted_iso = sorted([d.isoformat() for d in highlighted_dates])
    today_iso       = date.today().isoformat()
    current_iso     = current.isoformat()

    hl_js = str(highlighted_iso).replace("'", '"')

    calendar_html = f"""
<style>
  *{{box-sizing:border-box;}}
  body{{margin:0;padding:0;overflow:hidden;}}
  .dp-wrap{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:13px;position:relative;}}
  .dp-input{{width:100%;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;cursor:pointer;
    background:#fff;font-size:13px;font-weight:600;color:#0f172a;box-sizing:border-box;}}
  .dp-input:focus{{outline:none;border-color:#2563eb;box-shadow:0 0 0 2px #bfdbfe;}}
  .dp-popup{{display:none;position:fixed;z-index:9999;background:#fff;border:1px solid #e5e7eb;
    border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.14);padding:14px;width:280px;}}
  .dp-popup.open{{display:block;}}
  .dp-nav{{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;}}
  .dp-nav button{{background:none;border:none;cursor:pointer;font-size:18px;color:#475569;
    padding:2px 8px;border-radius:6px;line-height:1;}}
  .dp-nav button:hover{{background:#f1f5f9;}}
  .dp-month-label{{font-weight:800;font-size:14px;color:#0f172a;}}
  .dp-grid{{display:grid;grid-template-columns:repeat(7,1fr);gap:2px;}}
  .dp-dow{{text-align:center;font-size:11px;font-weight:700;color:#94a3b8;padding:4px 0;}}
  .dp-day{{text-align:center;padding:5px 2px;border-radius:50%;cursor:pointer;font-size:12px;color:#0f172a;width:32px;height:32px;
    display:flex;align-items:center;justify-content:center;margin:auto;}}
  .dp-day:hover{{background:#eff6ff;}}
  .dp-day.other-month{{color:#cbd5e1;}}
  .dp-day.has-data{{background:#2563eb;color:#fff;font-weight:800;}}
  .dp-day.has-data:hover{{background:#1d4ed8;}}
  .dp-day.selected{{background:#2563eb;color:#fff;font-weight:800;box-shadow:0 0 0 3px #93c5fd;}}
  .dp-day.today:not(.has-data):not(.selected){{outline:2px solid #93c5fd;outline-offset:-1px;}}
  .dp-day.disabled{{color:#e2e8f0;cursor:default;pointer-events:none;}}
</style>
<div class="dp-wrap">
  <input class="dp-input" id="dp-input" readonly value="{current_iso}" onclick="togglePop(event)"/>
  <div class="dp-popup" id="dp-popup">
    <div class="dp-nav">
      <button onclick="prevMonth(event)">&#8249;</button>
      <span class="dp-month-label" id="dp-label"></span>
...
<script>
const HIGHLIGHTED = new Set({hl_js});
const TODAY="{today_iso}", MAX="{today_iso}";
let sel=new Date("{current_iso}T00:00:00"), vy=sel.getFullYear(), vm=sel.getMonth();
const MN=["January","February","March","April","May","June","July","August","September","October","November","December"];
const DN=["Su","Mo","Tu","We","Th","Fr","Sa"];
function pad(n){{return String(n).padStart(2,'0');}}
function iso(y,m,d){{return `${{y}}-${{pad(m+1)}}-${{pad(d)}}`;}}
function render(){{
  document.getElementById('dp-label').textContent=MN[vm]+' '+vy;
  const g=document.getElementById('dp-grid');
  g.innerHTML=DN.map(d=>`<div class="dp-dow">${{d}}</div>`).join('');
  const first=new Date(vy,vm,1).getDay(), dim=new Date(vy,vm+1,0).getDate(), diPrev=new Date(vy,vm,0).getDate();
  const si=iso(sel.getFullYear(),sel.getMonth(),sel.getDate());
  for(let i=first-1;i>=0;i--){{const d=diPrev-i,s=iso(vy,vm-1,d);g.innerHTML+=cell(d,s,true);}}
  for(let d=1;d<=dim;d++){{const s=iso(vy,vm,d);g.innerHTML+=cell(d,s,false);}}
  const tot=first+dim,rem=tot%7===0?0:7-(tot%7);
  for(let d=1;d<=rem;d++){{const s=iso(vy,vm+1,d);g.innerHTML+=cell(d,s,true);}}
}}
function cell(d,s,other){{
  let c='dp-day';
  if(other)c+=' other-month';
  if(HIGHLIGHTED.has(s))c+=' has-data';
  const si=iso(sel.getFullYear(),sel.getMonth(),sel.getDate());
  if(s===si)c+=' selected';
  if(s===TODAY)c+=' today';
  if(s>MAX)c+=' disabled';
  return `<div class="${{c}}" onclick="pick('${{s}}')">${{d}}</div>`;
}}
function pick(s){{
  if(s>MAX)return;
  sel=new Date(s+'T00:00:00');vy=sel.getFullYear();vm=sel.getMonth();
  document.getElementById('dp-input').value=s;
  render();
...
function togglePop(e){{e.stopPropagation();const p=document.getElementById('dp-popup');
  if(!p.classList.contains('open')){{
    const r=document.getElementById('dp-input').getBoundingClientRect();
    p.style.top=(r.bottom+4)+'px';p.style.left=r.left+'px';
  }}
  p.classList.toggle('open');render();}}
function prevMonth(e){{e.stopPropagation();vm--;if(vm<0){{vm=11;vy--;}}render();}}
function nextMonth(e){{e.stopPropagation();vm++;if(vm>11){{vm=0;vy++;}}render();}}
document.addEventListener('click',function(){{document.getElementById('dp-popup').classList.remove('open');}});
</script>
"""
    returned = components.html(calendar_html, height=48, scrolling=False)
    if returned and isinstance(returned, str):
        try:
            parsed = date.fromisoformat(returned)
...
def _render_daily_proposals() -> None:  # noqa: C901
    ...
    # ---- Export (triggered by checked rows when button was clicked) ----
    if export_clicked:
