...
def _enrich_proposals(rows: list[dict], signal_date: date, db_role: str) -> list[dict]:
    ...
    Uses the most recent feature_date <= signal_date so today's proposals
    still show yesterday's features when today's haven't been written yet.
    Company name comes from a direct ticker_master query (guaranteed).
    """
    if not rows:
        return rows
    ...
    try:
        ...
        try:
            # ---- company names (direct, always available) ----
            cur = conn.execute(
            ...
            # ---- most recent feature_date <= signal_date for each ticker ----
            feat_date_sql = f"""
                SELECT f.ticker,
                ...
                WHERE f.rn = 1
            """
            cur2 = conn.execute(feat_date_sql, [signal_date] + tickers)
            cols = [d[0] for d in cur2.description]
            for row_data in cur2.fetchall():
                d = dict(zip(cols, row_data))
                feature_map[d["ticker"]] = d

            # ---- MACD histogram (column name varies by schema version) ----
            try:
                feat_cols = [c[0] for c in conn.execute("DESCRIBE daily_features").fetchall()]
                macd_col = next(
                ...
                if macd_col:
                    macd_sql = f"""
                        SELECT ticker, ROUND({macd_col}, 2) AS macd_hist
                        FROM (
                            SELECT *, ROW_NUMBER() OVER (
                                PARTITION BY ticker ORDER BY feature_date DESC
                            ) AS rn
                    ...
                    """
                    for t, m in conn.execute(macd_sql, [signal_date] + tickers).fetchall():
                        macd_map[t] = m
            except Exception:
                pass
        ...
        pass  # enrichment failure → show rows with blanks, not a crash
...
def _render_daily_proposals() -> None:  # noqa: C901
    ...
    # Filters defined first so signal_date is available when Run Pipeline fires
    ...
    with col_right:
        ...
        if latest:
            chip_label = f"Pipeline run: {latest.get('run_date','?')}" + " · " + f"{latest.get('status','')}"
        c1, c2 = st.columns([5, 3])
        c1.markdown(
            f'<div class="da-chip" style="white-space:nowrap;height:38px;'            f'display:flex;align-items:center">{chip_label}</div>',
            unsafe_allow_html=True,
        )
        run_clicked = c2.button("▶  Run Pipeline", key="dp_run", type="primary", use_container_width=True)
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "MACD hist":     st.column_config.TextColumn("MACD\nhist",     width=75),
...
def _do_run_pipeline(role: str, run_date: date | None = None) -> None:
    ...
    with st.spinner(f"Running pipeline for {effective_date}…"):
