...
def _date_picker_with_highlights(
    ...
    """st.date_input with a visual mini-calendar showing proposal dates as blue circles.

    Uses a Streamlit-native date_input for reliable date selection.
    Proposal dates are shown as a scrollable row of blue chips below the input,
    and clicking one sets that date. A compact always-visible mini-calendar
    (rendered via components.html, no popup needed) shows the current month
    with blue dots on proposal days.
    """
    ...
    # Native date input (reliable, keyboard-accessible)
    ...
    # Quick-pick chips for dates that have proposals
    if highlighted_dates:
        chips_html = " ".join(
            f'<span onclick="parent.document.querySelector(\'[data-testid=\\\'stDateInput\\\']\').dispatchEvent(new Event(\'click\'))" '            f'style="display:inline-block;background:{"#2563eb" if d == selected else "#dbeafe"};'            f'color:{"#fff" if d == selected else "#1d4ed8"};'            f'border-radius:999px;padding:3px 10px;font-size:12px;font-weight:700;'            f'cursor:pointer;margin:2px;white-space:nowrap">{d.strftime("%b %d")}</span>'
            for d in sorted(highlighted_dates, reverse=True)[:14]
        )
        # Build mini-calendar for current month of selection
        hl_iso = set(d.isoformat() for d in highlighted_dates)
        ...
        cells = ""
        # Blank cells for days before month start (Mon=0 based, but we use Sun=0)
        sun_offset = (first_weekday + 1) % 7
        for _ in range(sun_offset):
            cells += '<div style="width:30px;height:28px"></div>'

        for day in range(1, num_days + 1):
            d_iso = f"{year}-{month:02d}-{day:02d}"
            has_data = d_iso in hl_iso
            is_sel = d_iso == sel_iso
            is_today = d_iso == today_iso
            if is_sel:
                bg, color, fw = "#2563eb", "#fff", "800"
                extra = "border-radius:50%;"
            elif has_data:
                bg, color, fw = "#dbeafe", "#1d4ed8", "700"
                extra = "border-radius:50%;"
            elif is_today:
                bg, color, fw = "transparent", "#0f172a", "600"
                extra = "border-radius:50%;outline:2px solid #93c5fd;outline-offset:-1px;"
            else:
                bg, color, fw = "transparent", "#475569", "400"
                extra = ""
            cells += (
                f'<div style="width:30px;height:28px;display:flex;align-items:center;'                f'justify-content:center;font-size:12px;font-weight:{fw};'                f'background:{bg};color:{color};{extra}">{day}</div>'
            )

        dow_row = "".join(
            f'<div style="width:30px;text-align:center;font-size:10px;font-weight:700;'            f'color:#94a3b8">{d}</div>'
        ...
        <div style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;
          ...
          padding:10px;display:inline-block;margin-top:6px">
          <div style="text-align:center;font-weight:800;font-size:13px;
            color:#0f172a;margin-bottom:8px">{month_name}</div>
