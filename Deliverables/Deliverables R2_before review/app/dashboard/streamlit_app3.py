...
def _render_daily_proposals() -> None:  # noqa: C901
    ...
    with col_right:
        ...
        c1.markdown(
            f'<div class="da-chip" style="margin-top:4px;white-space:nowrap;height:38px;'
            f'display:flex;align-items:center">{chip_label}</div>',
            unsafe_allow_html=True,
        )
        run_clicked = c2.button("▶  Run Pipeline", key="dp_run", type="primary", use_container_width=True)
        export_clicked = False  # Export button removed per request
    ...
    # ---- Filter row: run date (calendar picker) ----
    ...
    hdr_l.markdown('<h3 style="margin:0;font-size:17px;font-weight:800">Proposed Stocks</h3>', unsafe_allow_html=True)
    export_clicked = hdr_r.button("⬇ Export CSV", key="dp_export_csv", use_container_width=True)
