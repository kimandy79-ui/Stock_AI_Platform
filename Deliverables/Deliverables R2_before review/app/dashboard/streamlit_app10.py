...
def _enrich_proposals(rows: list[dict], signal_date: date, db_role: str) -> list[dict]:
    ...
    # Fill missing company names from yfinance and cache in ticker_master
        ...
        try:
            ...
            for t in missing_names:
                try:
                    ...
                    pass
            # Write back to ticker_master so future runs don't re-fetch
            if fetched:
                try:
                    wconn = _dbm.connect(db_role)
                    try:
                        for t, name in fetched.items():
                            wconn.execute(
                                "UPDATE ticker_master SET company_name = ?, "
...
def _render_daily_proposals() -> None:  # noqa: C901
    ...
    st.markdown('<h3 style="margin:0 0 10px;font-size:17px;font-weight:800">Proposed Stocks</h3>', unsafe_allow_html=True)
    export_clicked = False  # no export button; export triggered via _do_export_proposals if needed
    ...
    edited_df = st.data_editor(
        ...
        hide_index=True,
        column_config={
            "✓":             st.column_config.CheckboxColumn("✓", default=True, width=30),
    ...
    # ---- Per-ticker document download ----
    st.markdown(
        '<div style="margin-top:8px;padding-top:8px;border-top:1px solid #eef2f7">',
        unsafe_allow_html=True,
    )
    doc_tickers = checked_tickers or all_tickers
    doc_col1, doc_col2 = st.columns([3, 2])
    with doc_col1:
        st.markdown('<span class="da-label">📄 Generate ticker selection document</span>', unsafe_allow_html=True)
        doc_ticker: str = st.selectbox(
            "doc_ticker_sel",
            options=doc_tickers,
            index=0,
            label_visibility="collapsed",
    ...
    with doc_col2:
        if st.button("📄 Generate & Download", key="btn_gen_doc", use_container_width=True):
            sel_row = next((r for r in enriched if r.get("ticker") == doc_ticker), None)
            if sel_row:
                doc_bytes, doc_filename = _build_ticker_document(
                    row=sel_row,
                    signal_date=signal_date,
                    db_role=role,
                    strategy_config_id=strategy_config_id or (sel_row.get("strategy_config_id") or ""),
                )
                st.download_button(
                    label=f"⬇ Download {doc_filename}",
                    data=doc_bytes,
                ...
                st.warning(f"No data found for {doc_ticker}.")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('</div>', unsafe_allow_html=True)

    # ---- Export CSV (triggered by checked rows when button was clicked) ----
    if export_clicked:
...
def _build_ticker_document(
    ...
    """Build a plain-text AI-analysis document for one ticker.

    Includes:
    - Proposal metadata (score, rank, RR, signal, sector)
    - All feature values from daily_features
    - Step 3 screening score and flags
    - Step 4 analysis (setup scores, stop/target prices)
    - Strategy config (tunable + hardcoded settings)
    - Comparison of each feature against relevant thresholds

    Returns (utf-8 bytes, filename).
    Filename format: YYMMDD_TICKER_STRATEGYNAME.txt
    """
    ...
    def kv(key: str, val: Any, note: str = "") -> None:
        note_str = f"  # {note}" if note else ""
        lines_out.append(f"  {key:<35} {val}{note_str}")
    ...
    kv("Strategy config",    strategy_config_id)
    kv("Generated at",       __import__("datetime").datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    sep()
    ...
    kv("Sector",             row.get("sector") or "—")
    kv("Industry",           row.get("industry") or "—")
    kv("Raw rank",           row.get("raw_rank") or "—")
    kv("Diversified rank",   row.get("diversified_rank") or "—")
    kv("Proposal score raw", row.get("proposal_score_raw") or "—")
    kv("Proposal score final", row.get("proposal_score_final") or "—")
    kv("In raw top-N",       row.get("in_raw_top_n") or False)
    kv("In diversified top-N", row.get("in_diversified_top_n") or False)
    kv("Diversification applied", row.get("diversification_applied") or False)
    kv("Rejection reason",   row.get("rejection_reason") or "none")
    kv("Mechanical explanation", row.get("mechanical_explanation") or "—")
    sep()

    # ------------------------------------------------------------------ #
    h2("TECHNICAL FEATURES (from daily_features)")
    kv("Price (close_adj)",  _fmt_price(row.get("price")))
    kv("EMA20-EMA50 spread", f"{row.get('ema_spread_pct') or '—'}%",    "Trend alignment; positive = uptrend")
    kv("RSI14",              row.get("rsi14") or "—",                   "50-70 = healthy bull momentum")
    kv("ROC20 %",            f"{row.get('roc20') or '—'}%",             "20-day price rate of change")
    kv("Volume ratio",       row.get("volume_ratio") or "—",            "vs 20-day avg; >1.0 = elevated")
    kv("ATR %",              f"{row.get('atr_pct') or '—'}%",           "Volatility as % of price")
    kv("Sector rel strength", f"{row.get('rel_strength_pct') or '—'}%", "vs sector benchmark ETF")
    sep()

    # ------------------------------------------------------------------ #
    # Fetch step3, step4, strategy config from DB
    step3_row: dict = {}
    step4_row: dict = {}
    config_json: dict = {}
    try:
        ...
        try:
            ...
            if r3:
                cols3 = [d[0] for d in conn.execute(
                    "SELECT * FROM step3_candidates LIMIT 0").description]
            ...
            if r4:
                cols4 = [d[0] for d in conn.execute(
                    "SELECT * FROM step4_analysis LIMIT 0").description]
        ...
        lines_out.append(f"  [DB fetch error: {exc}]")

    # ------------------------------------------------------------------ #
    if step3_row:
        h2("STEP 3 — SCREENING")
        for k, v in step3_row.items():
            if k not in ("ticker", "signal_date", "strategy_config_id", "run_id", "created_at"):
                kv(k, v)
    else:
        h2("STEP 3 — SCREENING")
        lines_out.append("  No step3_candidates row found for this ticker/date/strategy.")
    sep()

    # ------------------------------------------------------------------ #
    if step4_row:
        h2("STEP 4 — SETUP ANALYSIS")
        for k, v in step4_row.items():
            if k in ("setup_type", "setup_score", "breakout_quality_score",
                     "squeeze_score", "timing_score", "confirmation_score",
                     "estimated_rr", "stop_price_raw", "target_price_raw",
                     "earnings_penalty", "macro_penalty"):
                kv(k, v)
        expl = step4_row.get("explanation_json")
        if expl:
            try:
                expl_dict = _json.loads(expl) if isinstance(expl, str) else expl
                h2("STEP 4 — EXPLANATION DETAIL")
                for k, v in (expl_dict or {}).items():
                    kv(k, v)
            except Exception:
                pass
    else:
        h2("STEP 4 — SETUP ANALYSIS")
        lines_out.append("  No step4_analysis row found for this ticker/date/strategy.")
    sep()

    # ------------------------------------------------------------------ #
    if config_json:
        h2("STRATEGY CONFIGURATION")
        _TUNABLE_KEYS = [
            "universe.min_price", "universe.min_avg_dollar_volume_20d",
            "screening.min_rvol", "screening.min_screening_score",
            "step4.target_R", "diversification.top_n",
            "diversification.sector_max_positions",
            "diversification.industry_max_positions",
            "diversification.sector_penalty_factor",
            "diversification.industry_penalty_factor",
        ...
        h2("  Tunable settings")
        for path in _TUNABLE_KEYS:
            val = _nested(config_json, path)
            kv(path, val if val is not None else "—")

        h2("  Full config JSON")
        for line in _json.dumps(config_json, indent=2).splitlines():
            lines_out.append("  " + line)
    sep()

    # ------------------------------------------------------------------ #
    h2("THRESHOLD COMPARISONS")
    lines_out.append("  (Feature value vs strategy threshold)")
    sep()

    def compare(label: str, feature_val: Any, threshold: Any, direction: str = ">") -> None:
        if feature_val is None or threshold is None:
            kv(label, f"feature={feature_val}  threshold={threshold}  result=N/A")
            return
        try:
            fv, tv = float(feature_val), float(threshold)
            if direction == ">":
                passed = fv >= tv
            elif direction == "<":
                passed = fv <= tv
            ...
            kv(label, f"{fv:.2f}  {direction}= {tv:.2f}  [{result}]")
        except (TypeError, ValueError):
            kv(label, f"feature={feature_val}  threshold={threshold}")

    min_price   = _nested(config_json, "universe.min_price") if config_json else None
    min_rvol    = _nested(config_json, "screening.min_rvol") if config_json else None
    min_score   = _nested(config_json, "screening.min_screening_score") if config_json else None

    compare("Price >= min_price",              row.get("price"),         min_price,  ">")
    compare("RVOL >= min_rvol",                row.get("volume_ratio"),  min_rvol,   ">")
    compare("Screening score >= min_score",    step3_row.get("screening_score"), min_score, ">")
    sep()
...
def _do_run_pipeline(role: str, run_date: date | None = None) -> None:
    """Run the pipeline in a background thread while polling DB for step progress."""
    import threading
    import time as _time

    effective_date = run_date or date.today()

    # Step labels shown during execution
    ...
    while thread.is_alive():
        ...
        # Try to read current steps_completed from DB
        current_steps: list[str] = last_steps
        try:
            from app.database import duckdb_manager
            conn = duckdb_manager.connect(role, read_only=True)
            try:
                row = conn.execute(
                    "SELECT steps_completed FROM pipeline_runs "
                    "WHERE run_date = ? ORDER BY created_at DESC LIMIT 1",
                    [effective_date],
                ).fetchone()
                if row and row[0]:
                    import json as _json
                    current_steps = _json.loads(row[0]) if isinstance(row[0], str) else list(row[0])
        ...
        completed_count = len(current_steps)
        total_steps     = len(_STEP_LABELS)
        last_step       = current_steps[-1] if current_steps else None
        next_step       = None
        step_keys       = list(_STEP_LABELS.keys())
        if last_step and last_step in step_keys:
            idx = step_keys.index(last_step)
            if idx + 1 < len(step_keys):
                next_step = step_keys[idx + 1]

        if next_step:
            current_label = _STEP_LABELS.get(next_step, next_step)
        elif last_step:
            current_label = _STEP_LABELS.get(last_step, last_step) + " ✓"
        else:
