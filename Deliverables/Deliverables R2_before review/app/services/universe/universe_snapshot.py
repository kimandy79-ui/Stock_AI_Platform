...
def normalize_sector(raw_sector: Any) -> Any:
    """Normalize a provider raw sector to a canonical internal sector.

    Resolution order (M21 Config Management Addendum §8):

    1. exact match against ``constants.SECTOR_ALIAS_MAP``;
    2. case-insensitive match against the same map;
    3. pass-through if the value already is a canonical sector;
    4. otherwise return the raw value unchanged (unknown sectors are not
       dropped — they remain visible for later alias-map curation).

    ``None`` / empty values pass through unchanged so existing nullable-sector
    behavior (e.g. benchmark/ETF rows) is preserved.
    """
    if raw_sector is None or raw_sector == "":
        return raw_sector
    if not isinstance(raw_sector, str):
        return raw_sector
    alias_map = constants.SECTOR_ALIAS_MAP
    if raw_sector in alias_map:
        return alias_map[raw_sector]
    lowered = raw_sector.strip().lower()
    for raw, canonical in alias_map.items():
        if raw.lower() == lowered:
            return canonical
    if raw_sector in constants.CANONICAL_SECTORS:
        return raw_sector
    return raw_sector
        ...
        try:
            ...
            try:
                ...
                for ticker, info in accepted.items():
                    canonical_sector = normalize_sector(info.sector)
