...
def build_ticker_report(
    ...
    blank()
    line("  MARKET CONTEXT")
    _mr_known = market_reg and market_reg not in ("—", "None", None)
    _mr_note = (
        "bull=100pts; neutral=60pts; bear=20pts; high_risk/extreme_risk=0pts"
        if _mr_known
        else "UNKNOWN — market regime engine produced no output; market_score=0.0 (not neutral)"
    )
    kv("    Market regime", market_reg, _mr_note)
    blank()
    ...
    _mr_lower = str(market_reg).lower() if market_reg and market_reg != "—" else ""
    if _mr_lower in mk_map:
        mk_band = mk_map[_mr_lower]
        mk_note = ""
    else:
        mk_band = "0 (UNKNOWN — regime engine produced no output)"
        mk_note = "  # market_score=0.0; not scored as neutral"
    line(f"      market_regime={market_reg} → market score                = {mk_band}{mk_note}")
    ...
    if step4_row:
        ...
        # Determine setup-type weights for display
        _setup_weight_map = {
            "breakout":           (0.50, 0.30, 0.10, 0.10),
            "volatility_squeeze": (0.10, 0.50, 0.30, 0.10),
            "trend_pullback":     (0.10, 0.10, 0.50, 0.30),
            "trend_resume":       (0.10, 0.10, 0.40, 0.40),
            "high_tight_flag":    (0.40, 0.10, 0.40, 0.10),
            "momentum_extension": (0.40, 0.10, 0.40, 0.10),
            "unknown":            (0.25, 0.25, 0.25, 0.25),
        }
        _sw = _setup_weight_map.get(setup_type, (0.25, 0.25, 0.25, 0.25))
        _setup_score_note = (
            f"= {_sw[0]}×bq + {_sw[1]}×sq + {_sw[2]}×timing + {_sw[3]}×conf "
            f"+ earnings_penalty + macro_penalty  "
            f"[type-weighted for {setup_type}]"
        )
        kv("    setup_score (type-weighted + penalties)", _fmt(setup_score, 2), _setup_score_note)
        blank()
        ...
        # Read authoritative values from explanation_json (set by Step 4 engine).
        expl_for_earn = step4_row.get("explanation_json") or {}
        earn_status   = expl_for_earn.get("earnings_status") or "UNKNOWN"
        earn_pen_stored = step4_row.get("earnings_penalty")
        macro_pen_stored = step4_row.get("macro_penalty")
        days_earn  = expl_for_earn.get("days_to_earnings_bd")
        earn_avoid = _nested(config_json, "earnings.avoid_within_bd")
        earn_max   = _nested(config_json, "earnings.penalty_points_max")
        macro_flag = expl_for_earn.get("macro_event_risk_flag")
        macro_pen  = _nested(config_json, "macro_event_risk.penalty_points")

        kv("    Days to earnings (bd)", str(days_earn) if days_earn is not None else "— (unknown)")
        kv("    Earnings avoid window", str(earn_avoid) if earn_avoid is not None else "—")
        kv("    earnings_status",       earn_status)

        # Earnings penalty display — use stored value and status, not recalculated
        _earn_pen_display = _fmt(earn_pen_stored, 2) if earn_pen_stored is not None else "0"
        if earn_status == "EARNINGS_UNKNOWN":
            kv("    Earnings penalty applied",
               f"{_earn_pen_display}  (strategy-scaled penalty — earnings date unknown)",
               "aggressive=40% normal=75% conservative=100% of max penalty")
        elif earn_status == "EARNINGS_TODAY":
            kv("    Earnings penalty applied",
               f"{_earn_pen_display}  (full penalty — earnings today)")
        elif earn_status == "EARNINGS_INSIDE_WINDOW":
            kv("    Earnings penalty applied",
               f"{_earn_pen_display}  (proportional penalty — inside avoid window)")
        else:
            kv("    Earnings penalty applied",
               f"0  (earnings clear — outside avoidance window)")

        kv("    earnings_penalty (stored)", _fmt(earn_pen_stored, 2))
        kv("    Macro event risk flag",     _bool(macro_flag))
        kv("    Macro penalty points",      str(macro_pen) if macro_pen is not None else "—")
        kv("    macro_penalty (stored)",    _fmt(macro_pen_stored, 2))
        blank()
