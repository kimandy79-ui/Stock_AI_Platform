...
def _ensure_debug_db() -> str | None:
    """Apply the debug schema if ``debug.duckdb`` is missing.

    Reuses the Module 03 schema manager (no duplicated SQL/DDL). Targets the
    **debug** role only and never opens ``prod.duckdb``. Idempotent: returns
    ...
    if is_ok:
        print(f"Initialized debug.duckdb at {debug_path}.")
        return None
    return (
